...
function CarouselProvider({
  ...
  const handleSetItemsCount = (count: number) => {
    setItemsCount(count);
  ...
  return (
    <CarouselContext.Provider
      value={{
        ...
        itemsCount,
        setItemsCount: handleSetItemsCount,
        disableDrag,
      }}
    >
      {children}
...
function CarouselComponent({
  ...
  const eventListeners = useRef<Map<string, Set<() => void>>>(new Map());
  ...
  const handleIndexChange = (newIndex: number) => {
    ...
    // Trigger 'select' event when index changes
    const callbacks = eventListeners.current.get('select');
    if (callbacks) {
  ...
  useEffect(() => {
    if (setApi) {
      const api: CarouselApi = {
        ...
        canScrollNext: () => currentIndex < itemsCount - 1,
  ...
  }, [currentIndex, itemsCount, setApi]);

  const dragFree = opts?.breakpoints?.["(max-width: 768px)"]?.dragFree ?? false;

  return (
    <CarouselProvider
      initialIndex={currentIndex}
      onIndexChange={handleIndexChange}
      disableDrag={disableDrag || dragFree}
      onItemsCountChange={handleItemsCountChange}
    >
      <CarouselInternal className={className}>
        {children}
      </CarouselInternal>
    </CarouselProvider>
  );
