...
// ORB AI Color Palette
const colors = {
  50: "#E5E7EB", // orb-light
  100: "#A855F7", // orb-accent
  200: "#8B5CF6", // orb-purple
  300: "#3B82F6", // orb-blue
  400: "#121212", // orb-darker
  500: "#0A0A0A", // orb-dark
  600: "#1a1d18",
  700: "#2a2e26",
  800: "#0A0A0A",
...
export function HeroSection({ 
  ...
  useEffect(() => {
    ...
    words.forEach((word) => {
      word.addEventListener("mouseenter", () => {
        word.style.textShadow = "0 0 20px rgba(168, 85, 247, 0.5)";
      });
  ...
  return (
    <div
      className="min-h-screen bg-gradient-to-br from-[#0A0A0A] via-black to-[#121212] text-[#E5E7EB] font-primary overflow-hidden relative w-full"
    >
          ...
          >
            <div className="mb-4 md:mb-6">
              {mainTitle.split(" ").map((word, index) => (
                <span key={index} className="word" data-delay={1600 + index * 150}>
                  {index > 0 && " "}{word}
                </span>
              ))}
            </div>
            ...
            >
              {subtitle.split(" ").map((word, index) => (
                <span key={index} className="word" data-delay={2600 + index * 150}>
                  {index > 0 && " "}{word}
                </span>
              ))}
            </div>
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle, rgba(139, 92, 246, 0.13) 0%, transparent 100%)`,
        }}
      ></div>
    </div>
  );
